        ...
        return [
            ...
            DeceptionIndicator(
                ...
                patterns=[
                    ...
                    r"\b(stress|shock|trauma)\s+(of\s+the\s+situation\s+)?(affected|impacted|clouded)\s+(my\s+)?(memory|judgment|recall|thinking)\b",
                    r"\bupon\s+further\s+(reflection|thought|consideration)\b",
                    r"\bI\s+(may\s+have|might\s+have|could\s+have)\s+(underestimated|forgotten|overlooked|missed)\b",
                    r"\bthe\s+(confusion|stress|shock)\s+(at\s+the\s+time|of\s+it\s+all)\b",
                    r"\bI\s+apologize\s+for\s+(the\s+)?(confusion|oversight|error|mistake)\b",
                ],
                weight=1.7,
                category="Insurance",
                source="ddl"
            ...
            DeceptionIndicator(
                ...
                patterns=[
                    ...
                    r"\b(experiencing|facing|having)\s+(financial\s+)?(difficulties|hardship|problems|issues)\b",
                    r"\bfinancial\s+(difficulties|hardship|strain|pressure|problems)\b",
                    r"\b(can't|cannot)\s+(afford|wait|delay)\b",
                    r"\bI\s+would\s+(like|appreciate|need)\s+this\s+(matter\s+)?(resolved|settled)\b",
                ],
                weight=1.6,
                category="Insurance",
                source="ddl"
            ...
            DeceptionIndicator(
                ...
                patterns=[
                    ...
                    r"\b(rolex|omega|cartier|breitling|tag\s+heuer|patek)\b",
                    r"\b(designer|louis\s+vuitton|gucci|prada|chanel|hermes|burberry)\b",
                    r"\b(diamond|sapphire|ruby|emerald|platinum)\s+(ring|necklace|pendant|bracelet|watch|earrings)\b",
                    r"\b(OLED|4K|8K|latest\s+model|newest\s+model|top\s+spec|high\s+spec)\b",
                    r"\b(brand\s+new|barely\s+used|like\s+new|mint\s+condition|pristine)\b",
                ],
                weight=1.5,
                category="Insurance",
                source="ddl"
            ...
            DeceptionIndicator(
                name="Claim Revision Red Flags",
                description="Amending or revising claims with significant changes",
                patterns=[
                    r"\b(forgot|failed|neglected|omitted)\s+to\s+(mention|include|list|add)\b",
                    r"\b(need|want)\s+to\s+(clarify|revise|amend|update|correct)\b",
                    r"\bnot\s+\d+[\"']?\s*(but|actually)\s+\d+\b",
                    r"\b(revised|updated|amended|corrected)\s+(list|total|value|claim|items)\b",
                    r"\b(initially|originally)\s+(forgot|missed|overlooked|underestimated)\b",
                    r"\bI\s+(should|must|need\s+to)\s+(also\s+)?(mention|add|include)\b",
                    r"\bin\s+addition\s+to\s+(the\s+)?(above|previous|original)\b",
                    r"\b(also|additionally)\s+(stolen|taken|missing|lost)\b",
                ],
                weight=1.8,
                category="Insurance",
                source="ddl"
